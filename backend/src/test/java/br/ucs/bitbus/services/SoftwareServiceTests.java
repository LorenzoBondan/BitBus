...
public class SoftwareServiceTests {
    ...
    void setUp() {
        ...
        Software object = Factory.createSoftware();
        PageImpl<Software> page = new PageImpl<>(List.of(object));
    ...
    public void findAllShouldReturnPage() {
        ...
        Page<SoftwareDTO> result = service.findAllByName(name, pageable);
        Assertions.assertNotNull(result);
    }

    @Test
    public void findByIdShouldReturnObjectWhenExistingId() {
        SoftwareDTO result = service.findById(existingId);
        Assertions.assertNotNull(result);
    ...
    @Test
    public void insertShouldReturnSoftwareDtoWhenValidData() {
        SoftwareDTO SoftwareDTO = Factory.createSoftwareDTO();
        SoftwareDTO result = service.insert(SoftwareDTO);
        Assertions.assertNotNull(result);
        Assertions.assertEquals(SoftwareDTO.getNome(), result.getNome());
    }

    @Test
    public void updateShouldReturnSoftwareDtoWhenExistingId() {
        SoftwareDTO SoftwareDTO = Factory.createSoftwareDTO();
        SoftwareDTO result = service.update(existingId, SoftwareDTO);
        Assertions.assertNotNull(result);
        Assertions.assertEquals(SoftwareDTO.getNome(), result.getNome());
    }
    ...
    public void updateShouldThrowEntityNotFoundExceptionWhenNonExistingId() {
        Assertions.assertThrows(ResourceNotFoundException.class, () -> {
            SoftwareDTO SoftwareDTO = Factory.createSoftwareDTO();
            service.update(nonExistingId, SoftwareDTO);
        });
    }
